...
public interface ChequeoRepository extends CrudRepository<Chequeo, BigInteger> {
    ...
    String q2 = "SELECT (to_char(date_trunc('year', date(to_timestamp(fecha_registrada))), 'YYYY'))AS Ano, count(c.id) AS Cantidad FROM chequeo c WHERE date(to_timestamp(c.fecha_registrada))>=now() - interval '10 year' and c.es_entrada = true GROUP BY Ano ORDER BY Ano;";
    String q3 = "select count(c.id) as cantidad, sum(a.precio) as precio from chequeo c  inner join autobus a  on a.id = c.autobus_id where c.es_entrada=true and  date( to_timestamp(c.fecha_registrada)) >=  date(date_trunc('day', CURRENT_DATE - interval '2 day')) and date( to_timestamp(c.fecha_registrada)) <  date(date_trunc('day', CURRENT_DATE));";
    String q4 = "SELECT (to_char(date_trunc('month', date(to_timestamp(fecha_registrada))), 'MM-YYYY')) AS Mes , sum(a.precio) as " +
                "Ganancia FROM chequeo c  inner join autobus a  on a.id = c.autobus_id WHERE date( to_timestamp(c.fecha_registrada))>date(date_trunc('month', CURRENT_DATE)) - interval '1 year' and c.es_entrada = true GROUP BY Mes ORDER BY Mes;";
    ...
    String q8 = "SELECT count(c.id) as cantidad, r.nombre_corredor as ruta\n" +
            ...
            "WHERE date(to_timestamp(c.fecha_registrada)) >= date_trunc('month', current_date - interval '1' year) and date(to_timestamp(c.fecha_registrada))< date_trunc('month', current_date) and c.es_entrada = true  \n" +
